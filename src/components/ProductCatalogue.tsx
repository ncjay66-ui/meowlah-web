...
import { hasPublishedScore, money, purpose, recordShoppingEvent, words } from '@/lib/shopping';
...
function Catalogue() {
  ...
  const scope = params.get('scope') === 'all' ? 'all' : 'recommended';
  ...
  return <div className="shop-shell">
    ...
    <details className="shop-note shop-score-help"><summary>{w('Why might a product have no MeowLah Score?', '为什么有些商品没有 MeowLah 评分？', 'Mengapa sesetengah produk tiada Skor MeowLah?')}</summary><p>{w('Scores are shown only when a cat food has enough reliable nutrition-label and ingredient data for the relevant scoring track. Complementary foods use separate criteria. Missing data is never treated as zero; toys and supplies are not nutrition-rated.', '只有在猫粮具备足够可靠的营养标签与成分资料、可按对应规则评估时，才会显示评分。辅食采用独立标准。缺少资料不会按零分处理；玩具和日用品不做营养评分。', 'Skor dipaparkan hanya apabila makanan kucing mempunyai maklumat label nutrisi dan ramuan yang boleh dipercayai untuk kaedah penilaian yang berkenaan. Makanan pelengkap menggunakan kriteria berasingan. Maklumat tiada tidak dianggap sifar; mainan dan kelengkapan tidak dinilai dari segi nutrisi.')}</p></details>
    ...
    <dialog ref={dialog} className="shop-compare-dialog"><div className="shop-dialog-head"><h2>{w('Your shortlist', '你的候选清单', 'Senarai pilihan anda')}</h2><button autoFocus onClick={() => dialog.current?.close()} aria-label={w('Close comparison', '关闭对比', 'Tutup perbandingan')}>×</button></div><p className="shop-note">{w('Reference prices. Compare unit prices within one food type.', '参考价格。单位价格适合在同类食品中比较。', 'Harga rujukan. Banding harga unit bagi jenis yang sama.')}</p><div className="shop-table-scroll"><table><thead><tr><th>{w('Compare', '对比项目', 'Banding')}</th>{selected.map(p => <th key={p.id}>{p.name_en}</th>)}</tr></thead><tbody>{[
      ...
      [w('Delivery coverage', '配送范围', 'Liputan penghantaran'), (p: Product) => marketDeliveryNote(marketReview(p.id),lang)],
      ...
      [w('Nutrition score', '营养评分', 'Skor nutrisi'), (p: Product) => isCatSupply(p) ? w('Not scored', '不适用', 'Tidak dinilai') : hasPublishedScore(p) ? `${Math.round(p.final_score!)}/100 · ${p.grade}` : w('Score pending', '评分待完善', 'Skor belum tersedia')],
