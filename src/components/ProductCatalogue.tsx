...
function Catalogue() {
  ...
  return <div className="shop-shell">
    ...
    <nav className="shop-quick-links" aria-label={w('Shop by category', '按品类浏览', 'Beli mengikut kategori')}>{(['wet','dry','toys','litter'] as const).map(c => <button key={c} onClick={() => { change({ category: c, scope: 'all' }); document.getElementById('shop-results')?.scrollIntoView({ behavior: 'smooth' }); }}>{getCategoryLabel(c, lang)} →</button>)}</nav>
